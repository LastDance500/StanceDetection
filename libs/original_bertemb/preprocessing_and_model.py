...
from STANDER.libs.original_bertemb.models import WingOsModel
...
def preprocess_bert(data_bert, bert_matrix, stances, op=None, only_stance=False, number_sents=5, no_classes=3):
	all_data=load_json('final_merged_annotations_correct.json')
	x_train, x_test = [[], [], [], [], [], [], [], []], [[], [], [], [], [], [], [], []]
	train_gold_ev, test_gold_ev = [], []

	target_embs = numpy.load("bert_target_embeddings.npy")
	...
	for file_id, file_content in data_bert.items():
		...
		if only_stance:
			# get gold sentences
			s, e = file_content["gold_sentences_indices"][0], file_content["gold_sentences_indices"][1]
		else:
			# get all sentences
