...
class NewMetrics(Callback):
	...
	def __init__(self, train_data, train_labels, dev_data, dev_labels, train_gold_ev, test_gold_ev, only_stance, reversed_stance_dict, number_sents=5):
	...
	def on_epoch_end(self, batch, logs={}):
		...
		if not self.only_stance:
			...
			val_er_r, val_er_p = self.evidence_rec(self.test_gold_ev, pred_evidences), self.evidence_prec(self.test_gold_ev, pred_evidences)
		...
		print(" — f1: ", round(_f1, 4), " — precision: ", round(_precision, 4), " — recall", round(_recall, 4),\
			" — val_f1: ", round(_val_f1, 4), " — val_precision: ", round(_val_precision, 4), " — val_recall", round(_val_recall, 4))
...
class WingOsModel():
	...
	def build_model(self):
		...
		# --- compile model ---
		# define variables
		inputs = [_input_target, _input_sentence_1, _input_sentence_2, _input_sentence_3, _input_sentence_4, _input_sentence_5]
		
		if not self.only_stance:
			outputs = [stance_probs, alpha_1, alpha_2, alpha_3, alpha_4, alpha_5]
			self.model = Model(inputs=inputs, outputs=outputs)
			self.loss = {"stance_probs": "categorical_crossentropy", "alpha_1": "binary_crossentropy", "alpha_2": "binary_crossentropy", "alpha_3": "binary_crossentropy", "alpha_4": "binary_crossentropy", "alpha_5": "binary_crossentropy"}
			self.metrics = [categorical_accuracy, binary_accuracy, binary_accuracy, binary_accuracy, binary_accuracy, binary_accuracy]
			...
			self.model.compile(metrics = [categorical_accuracy], optimizer = tf.optimizers.Adam(learning_rate=0.02), loss = self.loss)
			
			self.model.compile(loss='categorical_crossentropy', optimizer=tf.optimizers.Adam(learning_rate=0.02), metrics=['accuracy'])
